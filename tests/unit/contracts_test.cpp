...
TEST_CASE("config migrations require explicit microphone selection and retain the model default") { auto parsed = modernime::parseConfigSnapshot("{\"protocol_version\":1}"); REQUIRE(parsed); REQUIRE(parsed->microphone.empty()); REQUIRE(parsed->model_id == "sherpa-onnx-streaming-zipformer-bilingual-zh-en-2023-02-20"); }
